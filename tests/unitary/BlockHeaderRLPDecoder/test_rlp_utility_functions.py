...
def test_skip_rlp_list_header(block_headers_decoder_module):
    """
    ...
    current_pos = 0
    result = block_headers_decoder_module.eval(
...
def test_skip_rlp_string(block_headers_decoder_module):
    """
    ...
    result = block_headers_decoder_module.eval(f"self._skip_rlp_string({single_byte}, 0)")
    ...
    result = block_headers_decoder_module.eval(f"self._skip_rlp_string({short_string}, 0)")
    ...
    result = block_headers_decoder_module.eval(f"self._skip_rlp_string({max_short_string}, 0)")
    ...
    result = block_headers_decoder_module.eval(f"self._skip_rlp_string({boundary_case}, 0)")
    ...
    result = block_headers_decoder_module.eval(f"self._skip_rlp_string({long_string_header}, 0)")
    assert result == 12, "Long string with 1-byte length field should advance pos by 2 + length"


def test_read_rlp_number(block_headers_decoder_module):
    """
    Test _read_rlp_number function with various inputs
    """
    # Test case 1: Single byte number
    single_byte = bytes([0x7F])
    value, next_pos = block_headers_decoder_module.eval(f"self._read_rlp_number({single_byte}, 0)")
    ...
    value, next_pos = block_headers_decoder_module.eval(f"self._read_rlp_number({short_number}, 0)")
    ...
    value, next_pos = block_headers_decoder_module.eval(
        f"self._read_rlp_number({two_byte_number}, 0)"
    )
    assert value == 256, "Should decode two-byte number correctly"
    assert next_pos == 3, "Should advance position by 3"


def test_read_hash32(block_headers_decoder_module):
    """
    ...
    hash_value, next_pos = block_headers_decoder_module.eval(f"self._read_hash32({hash_bytes}, 0)")
